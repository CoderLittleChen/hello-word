...
/****** Object:  StoredProcedure [dbo].[P_WorkHourSummaryView]    Script Date: 2020/2/28 14:12:16 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN
		...
		if(@SysProjectManager=1)
			BEGIN
				...
				SELECT ProCode  FROM  ProductInfo WHERE    Manager=@UserId  and DeleteFlag!=1; 
			END
		--是POP
		else if(@SysPoP=1)
			BEGIN
...
--select  SUM(Percents)  from  #temp_table;
--select  distinct  *  from   #tempCheckedProductInfo  ;
--select     *  from   #tempProductInfo;
--select  *  from   #tempDeptInfo;
--select whd.*  from  WorkHourDetail  whd   inner join  #tempCheckedProductInfo  a   on  whd.ProCode=a.Pro_Code
--where whd.YearMonth=202002;
