	...
	begin
		...
		begin try
			begin transaction 
				...
				--插入解决方案   有就不插入
				--if(select  COUNT(*)  from  Sol_DataIDSet where SrcID=1)=0
				--	begin
				--		insert  into   Sol_DataIDSet
				--		select  1,0,'解决方案',0,0,1,0,1;
				--	end
				...
				--导入R版本
				MERGE Sol_DataIDSet  AS T
				USING (SELECT pro.Release_Code,pro.Release_Name,pro.solution_Code FROM Sol_ProductInfo pro GROUP BY pro.Release_Code,pro.Release_Name,pro.solution_Code)  AS S
				ON T.SrcID=S.Release_Code
				WHEN MATCHED THEN 
					UPDATE SET T.SrcName=S.Release_Name,T.SrcPID=S.solution_Code
				WHEN NOT MATCHED THEN 
					INSERT ([SrcID],[SrcPID],[SrcName],[IDLevel],[OrderNo],[Status],[DeleteFlag],[Show]) 
					VALUES(S.Release_Code,S.solution_Code,S.Release_Name,4,0,1,0,1);
				...
				WHEN NOT MATCHED THEN 
					INSERT ([SrcID],[SrcPID],[SrcName],[IDLevel],[OrderNo],[Status],[DeleteFlag],[Show]) 
					VALUES(S.B_Code,S.Release_Code,S.B_Name,5,0,1,0,1);
				...
				insert into  Sol_Permission
				(UserType,UserName,DataSetID,RCode,CreateBy,CreateTime,Modifier,ModifyTime)
				select  1 as userType,SUBSTRING(b.STDMgr,CHARINDEX(' ',b.STDArch)+1,10),a.DataSetID,1,'',GETDATE(),null,null  from  Sol_DataIDSet  a  
				inner join Sol_ProductInfo b on  a.SrcID=b.D_Code
