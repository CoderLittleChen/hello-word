...
WITH specEntry AS 
(
	SELECT   dbo.specMS_SpecEntry.nodes, dbo.specMS_SpecEntry.entryID, dbo.specMS_SpecEntry.entryPID, 
...
Module AS
    (
	SELECT   dbo.specMS_SpecModuleBLRel.blID, A.smID, A.smPID, A.smName, A.smLvl, A.mCode, A.explain, A.fspecNum, 
                     A.type, A.verTreeCode, A.mmanagerID, A.mmanager, A.orderNo, A.createBy, A.createTime, A.Release_Name
     FROM      dbo.specMS_SpecModuleBLRel WITH (NOLOCK) 
	 LEFT OUTER JOIN
                         (
							SELECT distinct  dbo.specMS_SpecModule.smID, dbo.specMS_SpecModule.smPID, dbo.specMS_SpecModule.smName, 
	 ...
	 LEFT OUTER JOIN
                         (
							SELECT distinct  dbo.specMS_SpecModule.smID, dbo.specMS_SpecModule.smPID, dbo.specMS_SpecModule.smName, 
                                          ...
                                          dbo.specMS_SpecModule.mmanager, dbo.specMS_SpecModule.orderNo, 
                                          dbo.specMS_SpecModule.createBy, dbo.specMS_SpecModule.createTime, 
                                          dbo.Sync_ProductInfo.BVersionName as Release_Name
                             FROM    dbo.specMS_SpecModule WITH (NOLOCK) LEFT OUTER JOIN
                                          dbo.Sync_ProductInfo WITH (NOLOCK) ON 
...
(
	...
	FROM      (SELECT   list.blID, idset.srcName AS EntryModifyVer
            ...
            FROM      dbo.specMS_SpecModule AS module INNER JOIN
                     ...
                     dbo.specMS_SpecBaseLine AS bl ON main.blID = bl.blID
),Template AS
(
...
select    *  from  Module  order  by blID,smID ;
