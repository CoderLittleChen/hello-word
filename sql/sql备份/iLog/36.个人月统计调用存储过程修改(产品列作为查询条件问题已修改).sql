...
/****** Object:  StoredProcedure [dbo].[P_PersonalMonthView]    Script Date: 2020/3/19 15:49:46 ******/
SET ANSI_NULLS ON
GO
        ...
        FROM    dbo.DateSetting a
                LEFT JOIN ( SELECT  hm.BillNo ,
                            ...
                            FROM    dbo.HourInfoMain hm
                                    INNER JOIN dbo.V_HourInfo_New hi ON hi.HMain_ID = hm.id
        ...
        FROM    dbo.DateSetting a
                LEFT JOIN ( SELECT  hm.BillNo ,
                            ...
                            FROM    dbo.HourInfoMainHistory hm
                                    INNER JOIN dbo.V_HourInfoDetailHistory hi ON hi.HMain_ID = hm.id
                                    LEFT JOIN ProductInfo_Display p ON p.Current_ProId=hi.ProID
                            WHERE   hm.UserCode = @code
                                    AND SUBSTRING(CONVERT(NVARCHAR(200), hi.Date, 23),
                                                  0, 8) = @month
        ...
        IF ( @ProjectLevel = '2' )
            BEGIN
                SET @SQL = 'SELECT ROW_NUMBER() OVER( ORDER BY Date,HMain_ID) RowID, SecondDeptName , DeptName,UserName,UserCode,
										...
										'''' ThirdProCode ,Convert(uniqueidentifier,''00000000-0000-0000-0000-000000000000'') FourthProID,'''' FourthProName,'''' FourthProCode,ProCode,    
            ...
            IF ( @ProjectLevel = '3' )
                BEGIN
                    SET @SQL = 'SELECT ROW_NUMBER() OVER( ORDER BY Date,HMain_ID) RowID, SecondDeptName , DeptName,UserName,UserCode,
										...
										'''' FourthProName,'''' FourthProCode,ProCode,    
                                        DeptCode,SecondDeptCode,ProductLineCode,Station FROM  #AllData  '
                        + @conWhere
                        + '  GROUP BY SecondDeptName , DeptName,UserName,UserCode,
                                        Date,FillState,IsReview,BillNo,HMain_ID ,IsLeave,StationName,
                                        ProductLineName , PDTName , PDTCode,FirstProID, FirstProCode  ,FirstProName ,SecondProID ,SecondProCode  , 
                                        SecondProName  , ThirdProID, ThirdProName ,ThirdProCode,ProCode,  
                                        DeptCode,SecondDeptCode,ProductLineCode,Station,ReMark;';                    
                END;	
            ELSE
                IF ( @ProjectLevel = '4' )
                    BEGIN
                        SET @SQL = 'SELECT ROW_NUMBER() OVER( ORDER BY Date,HMain_ID) RowID, SecondDeptName , DeptName,UserName,UserCode,
                                        ...
                                        ProductLineName , PDTName , PDTCode,FirstProID, FirstProCode  ,FirstProName ,SecondProID ,SecondProCode  , 
                                        SecondProName ,ThirdProID, ThirdProName ,ThirdProCode,FourthProID, FourthProName,FourthProCode,ProCode,  
                                        DeptCode,SecondDeptCode,ProductLineCode,Station FROM  #AllData  '
                            + @conWhere
                            --+ '  GROUP BY SecondDeptName , DeptName,UserName,UserCode,
                ...
                ELSE
                    BEGIN
                        SET @SQL = 'SELECT ROW_NUMBER() OVER( ORDER BY Date,HMain_ID) RowID, SecondDeptName , DeptName,UserName,UserCode,
                            ...
                            + '  GROUP BY SecondDeptName , DeptName,UserName,UserCode,
                                        Date,FillState,IsReview,BillNo,HMain_ID ,IsLeave, StationName,
                                        ProductLineName , PDTName , PDTCode,FirstProID, FirstProCode  ,FirstProName,ProCode,   
                                        DeptCode,SecondDeptCode,ProductLineCode,Station,ReMark;'; 
