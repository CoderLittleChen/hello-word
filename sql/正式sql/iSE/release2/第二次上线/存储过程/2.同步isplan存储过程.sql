...
/****** Object:  StoredProcedure [dbo].[P_SyncSolProductInfo]    Script Date: 2021/1/27 9:47:41 ******/
SET ANSI_NULLS ON
GO
...
-- =========================================================
Alter procedure  [dbo].[P_SyncSolProductInfo]
as
	--事务中操作的错误记录
	declare @error int =0
	begin
		...
		begin try
			begin transaction 
				...
				--更新版本的OrderNo
				if (select COUNT(*)  from  Sol_DataIDSet where  OrderNo=0)>0
					begin		
						update  Sol_DataIDSet  set OrderNo=temp.RankNum
						from  (select  *,ROW_NUMBER() over(partition by srcPid  order by  srcId) as RankNum  from  Sol_DataIDSet a) temp
						where Sol_DataIDSet.SrcID=temp.SrcID
					end
