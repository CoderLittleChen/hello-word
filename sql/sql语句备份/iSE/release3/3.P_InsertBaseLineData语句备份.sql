...
/****** Object:  StoredProcedure [dbo].[P_InsertBaseLineData]    Script Date: 2020-10-28 13:53:45 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[P_InsertBaseLineData]
(
...
BEGIN TRY
	--BEGIN TRANSACTION TRANS
		...
		BEGIN		
			...
			SELECT @COUNT=COUNT(*) FROM SPECMS_SPECBASELINE WHERE PREBLID=@Parentid AND (STATUS=0 OR STATUS=-1)
					AND BLID IN (SELECT BLID FROM SPECMS_SPECLIST WHERE VERTREECODE=@VerRCode);
			SELECT @COUNT2=COUNT(*) FROM SPECMS_SPECBASELINE WHERE BLID=@Parentid AND (STATUS=0 OR STATUS=-1)
					AND BLID IN (SELECT BLID FROM SPECMS_SPECLIST WHERE VERTREECODE=@VerRCode ) ;
